export default class od6sMiscRulesConfiguration extends FormApplication {
    ...
    static get defaultOptions() {
        ...
        options.id = "custom_miscrules";
        options.template = "systems/od6s/templates/settings/settings.html";
        options.width = 600;
        options.minimizable = true;
        options.resizable = true;
        options.title = game.i18n.localize("OD6S.CONFIG_MISC_RULES");
    ...
    getData() {
        ...
        data.settings = Array.from(game.settings.settings).filter(s => s[1].od6sMiscRules).map(i => i[1])

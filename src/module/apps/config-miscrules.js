...
export default class od6sMiscRulesConfiguration extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    static DEFAULT_OPTIONS = {
        ...
        actions: {
            submit: od6sMiscRulesConfiguration.#onClose
        }
    };
    ...
    async _prepareContext(options) {
        const context = {};
        context.settings = Array.from(game.settings.settings).filter(s => s[1].od6sMiscRules).map(i => i[1]);

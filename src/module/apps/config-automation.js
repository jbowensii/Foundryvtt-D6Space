...
export default class od6sAutomationConfiguration extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    static DEFAULT_OPTIONS = {
        id: "od6s-config-automation",
        classes: ["od6s", "settings"],
        tag: "form",
        position: { width: 600, height: "auto" },
        window: {
            title: "OD6S.CONFIG_AUTOMATION_OPTIONS_MENU",
            resizable: true,
        ...
        actions: {
            submit: od6sAutomationConfiguration.#onClose
        }
    };
    ...
    async _prepareContext(options) {
        const context = {};
        context.settings = Array.from(game.settings.settings).filter(s => s[1].od6sAutomation).map(i => i[1]);
